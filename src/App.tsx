...
const App = () => {
  ...
  const fetchWeather = async (city: string): Promise<void> => {
    ...
    const requestUrl = `${baseUrl}?key=${API_KEY}&q=${encodeURIComponent(city)}&aqi=no`;
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Error on fetching data (${response.statusText})`);
      }
      ...
      if (error instanceof Error) {
        setOutputWeather(`Error on fetching data, ${error.message}`);
      } else {
